...
@NamedNativeQueries({
	@NamedNativeQuery(
	name = "getProjectNo",
	query = "SELECT DISTINCT * FROM (SELECT DISTINCT * "
			+ "FROM WFD0000_PROJECT_IDS "
			+ "WHERE STATUS = 'A' "
			+ "UNION "
			+ "SELECT DISTINCT * "
			+ "FROM WFD0000_PROJECT_IDS "
			+ "WHERE STATUS     = 'I' "
			+ "AND STATUS_DATE >= to_date('01/01/' ||(TO_CHAR(sysdate,'YYYY')-3),'DD/MM/YYYY') AND PROJECT_ID NOT IN (SELECT DISTINCT PROJECT_ID "
			+ "FROM WFD0000_PROJECT_IDS "
			+ "WHERE STATUS = 'A')) "
			+ "ORDER BY PROJECT_ID",
        resultClass = ProjectId.class
	)
})
